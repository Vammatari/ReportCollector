...
class AppendixTable(Table):
    ...
    def job(path_to_load: str, path_to_save: str) -> None:
        try:
            ...
            for sheet in sheets:
                ...
                AppendixTable.__fill_with_one_sheet(df, requests['Запросы'])
                requests.save(path_to_save)
        except PermissionError:
            pass

    @staticmethod
    def __make_row(df: pd.DataFrame, row: int, i: int, year: int, project: str, area: str, stage: str) -> list:
        ...
        return [
                ...
                stage, # стадия работ
                project + '-' + area, # проект-участок
                df.iloc[row,_JOBS] + ', ' + df.iloc[row,_UNITS]
                ]

    @staticmethod
    def __make_row_year(df: pd.DataFrame, row: int, year: int, project: str, area: str, stage: str) -> list:
        return [df.iloc[row,_JOBS], df.iloc[row,_UNITS], year, df.iloc[row,_YEAR_PLAN], project, area, stage, project + '-' + area, df.iloc[row,_JOBS] + '-' + str(df.iloc[row,_UNITS])]
                
    @staticmethod
    def __fill_with_one_sheet(df: pd.DataFrame, sheet: Worksheet) -> None:
        project, area, stage, year = AppendixTable.__get_service_info(df)
        query = DBConnection()
        for row in range(13, 141):
            if row in _rows_to_skip:
                continue
            if str(df.iloc[row,2]) != 'nan' and df.iloc[row,2] != 0:
                new_row = AppendixTable.__make_row_year(df,row,year,project,area,stage)
                print(new_row)
            ...
            for i in range(0, len(_month_indices), 2):           
                ...
                # if (plan != 0 and str(plan) != 'nan'): # or (df.iloc[row,2] != 0 and str(df.iloc[row,2]) != 'nan'):
                    new_row = AppendixTable.__make_row(df, row, i, year, project, area, stage)
                    print(new_row)                    
                ...
                # if (str(fact) != 'nan' and fact != 0): # or (df.iloc[row,2] != 0 and str(df.iloc[row,2]) != 'nan'):
                    new_row = AppendixTable.__make_row(df, row, i + 1, year, project, area, stage)
    ...
    @staticmethod
    def __get_service_info(df: pd.DataFrame) -> list[str, str, str, int]:
        ...
        return project, area, stage, int(year)